...
public class AlmacenComida implements Runnable{
	...
	public synchronized boolean solicitarComida(int cantidadComida,String ciudad) throws InterruptedException {
		if(cantidadComida<=this.totalComida) {
			int comidasolicitada = (int) Math.floor(Math.random()*10);//simulamos la preparación de la comida multiplicando la cantidad solicitada por 10 milisegundos
			totalComida = totalComida - comidasolicitada; //restamos del total de comida la comidasolicitada
